...
void matrix_show_q1(mat m)
{
	for(int i = 0; i < m->m; i++) {
		for (int j = 0; j < m->n; j++) {
			
shm.viv1[i][j]= m->v[i][j];
...
void householder(mat m, mat *R, mat *Q,int a, int b)
{
	mat q[m->m];
	mat z = m, z1;
	for (int k = a; k <b; k++) {
...
int main()
{
	...
	c1[1]=1;
	mat R, Q;
	mat x = mat_cpy(4, in,4);
//	householder(x, &R, &Q,e[1],d[1]);
if(row==0 && col==0){
householder(x,&R,&Q,0,2);	
matrix_show_q(Q);
}
if(row==0 && col==1){
householder(x,&R,&Q,2,4);
matrix_show_q1(Q);
}
shm.flag[row+col]=1;
	while(1);
